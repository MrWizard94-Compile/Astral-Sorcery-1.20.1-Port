...
public class ItemDazzlingGem extends ItemAS {
    ...
    public static boolean setQuality(ItemStack stack, GemQuality quality) {
        if (stack.isEmpty() || !(stack.getItem() instanceof ItemDazzlingGem)) return false;
    ...
    public static Optional<GemQuality> getQuality(ItemStack stack) {
        if (stack.isEmpty() || !(stack.getItem() instanceof ItemDazzlingGem)) return Optional.empty();
        CompoundTag tag = NBTHelper.getPersistentData(stack);
