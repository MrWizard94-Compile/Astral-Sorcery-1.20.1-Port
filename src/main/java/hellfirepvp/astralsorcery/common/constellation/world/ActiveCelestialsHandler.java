...
/**
 * Tracks the screen-space render positions for active constellations in the night sky.
 * Positions are pre-computed UV-quad corners so the sky renderer can batch-draw each
 * constellation at a consistent location without recalculating every frame.
 *
 * <p>Hard cap: 15 constellations can appear simultaneously (enough for all AS constellations).</p>
 */
public class ActiveCelestialsHandler {

    public static final LinkedList<RenderPosition> availablePositions = new LinkedList<>();
